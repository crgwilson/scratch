# Container Orchestration

<!-- tagged-notes:start -->
## Tagged Notes

- [kubernetes/README](../kubernetes/README.md)
- [kubernetes/application-deployment](../kubernetes/application-deployment.md)
- [kubernetes/authentication-authorization](../kubernetes/authentication-authorization.md)
- [kubernetes/autoscaling](../kubernetes/autoscaling.md)
- [kubernetes/custom-metrics](../kubernetes/custom-metrics.md)
- [kubernetes/helm](../kubernetes/helm.md)
- [kubernetes/installation](../kubernetes/installation.md)
- [kubernetes/kubectl](../kubernetes/kubectl.md)
- [kubernetes/storage](../kubernetes/storage.md)
<!-- tagged-notes:end -->

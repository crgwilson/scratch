...
## re: Basic usage
Compile a regex pattern using `re.compile()`, and then search for that pattern using `pattern.match()`.

You can search for multiple patterns and return all matches as a dict by labelling your patterns in your regex with `?P<some-name>`, calling `groupdict()` on your `match()` results.
